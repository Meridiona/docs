...
This opens the repo-root `.env` in your `$EDITOR`. Add or update the Jira block:
...
<Steps>
  <Step title="Restart the daemon">
    ...
    meridian restart
    ```
    The daemon re-reads the repo-root `.env` on startup and refreshes the `pm_tasks` cache from Jira within the first poll cycle.
  </Step>
...
The script reads your credentials from the repo-root `.env` automatically — no extra configuration needed.
